...
def define_role_structure(group: Group):
    ...
    group.roleList = str(['I3', 'I2', 'I1', 'I0', 'inactive'])  # ordered accourding to importance, i.e., the are filled accordingly
    est_num_traders = num_traders(group=group)
    group.estNumTraders = est_num_traders
    num_participants = group.numParticipants
    num_I3 = int(round(1/4*num_participants, 0))  # informed about 50c, 20c, and 10c
    num_I2 = int(round(1/4*num_participants, 0))  # informed about 2E, and 2c
    num_I1 = int(round(1/4*num_participants, 0))  # informed about 1E, and 5c
    num_I0 = max(0, num_participants - num_I3 - num_I2 - num_I1)  # uninformed traders
    num_inactive = 0
    group.roleStructure = str({'inactive': num_inactive, 'I0': num_I0, 'I1': num_I1, 'I2': num_I2, 'I3': num_I3})
...
def get_role_attr(player: Player, role_id):
    ...
    if role_id == 'inactive':
        ...
        for partition in PARTITIONS_NAMES:
            if role_info_structure[role_id][partition] == 0:
